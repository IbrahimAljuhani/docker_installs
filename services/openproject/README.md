...
You'll also be asked whether to publish a host port for direct access without NPM (e.g. `http://<server-ip>:8080`) — useful for a quick first check that the container actually works before wiring up NPM. Default is no. Say yes and the port is checked for conflicts, then printed as a URL once OpenProject starts. **Two caveats if you use it**: only `web` is reachable this way — the `/hocuspocus` real-time collaborative-editing path still needs NPM's routing — and since `OPENPROJECT_HTTPS=true` is set for eventual NPM/TLS use, some behavior (secure cookies, redirects) may not be fully correct over plain `http://` until NPM/SSL is set up. Change the port later the same way as the memory limit: edit `HOST_PORT=` in `.env` and rerun `deploy.sh`.
