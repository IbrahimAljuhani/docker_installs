#!/bin/bash
# deploy.sh (services/Web/wordpress)
# Purpose: Deploy WordPress (app + MySQL) — see docker-compose.yml for the
# full stack and the deliberate deviations from the official WordPress
# Docker Hub image's own docker-compose example.
#
# This is a single-instance service: one WordPress deployment per host,
# under ~/docker/wordpress/.
...
if [[ $# -gt 0 ]]; then
    case "$1" in
        --help|-h)
            ...
            echo "Deploy WordPress behind the shared 'main-net' network."
            exit 0
            ;;
    esac
fi
...
INSTALL_DIR="$HOME/docker/wordpress"
LOGFILE="$INSTALL_DIR/deploy.log"
SECRETS_FILE="$INSTALL_DIR/.wordpress-docker-secrets.txt"
...
if [[ -f "$INSTALL_DIR/.env" ]]; then
    ...
    prompt_mem_limit "wordpress-app" "512m"
...
EOF
    ...
    {
        echo "# Auto-generated WordPress secrets - DO NOT SHARE"
        echo "$(date '+%F %T')"
...
if [[ -n "$ENV_MEM_LIMIT" || -n "$ENV_HOST_PORT" ]]; then
    ...
    [[ -n "$ENV_MEM_LIMIT" ]] && print_info "Memory limit $ENV_MEM_LIMIT applied to the 'wordpress-app' container (db stays unbounded)."
...
(cd "$INSTALL_DIR" && $COMPOSE_CMD up -d 2>&1 | tee -a "$LOGFILE") \
    || print_error "Failed to start WordPress. Check log: $LOGFILE"

print_info "WordPress is starting."
echo
echo "──────────────────────────────────────────────"
if [[ -n "$ENV_HOST_PORT" ]]; then
...
echo "🔗 Proxy target: wordpress-app:80 on 'main-net'"
echo "👤 First visit:  follow WordPress's own setup wizard — create your own admin account"
echo "📜 Log:          $LOGFILE"
[[ -f "$SECRETS_FILE" ]] && echo "🔒 Secrets:      $SECRETS_FILE"
echo "──────────────────────────────────────────────"
echo
echo "Set up NGINX Proxy Manager: forward to wordpress-app, port 80, enable SSL."
