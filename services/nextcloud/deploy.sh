...
# redis, cron stay unbounded). Sets MEM_LIMIT in the caller's shell (no
# command substitution — keeps the prompts on a real terminal instead of
# risking them being swallowed into a captured value).
MEM_LIMIT=""
prompt_mem_limit() {
    ...
    [[ "${answer,,}" == "y" ]] || return 0
    while true; do
        read -rp "Memory limit (default: $default, e.g. 1g, 512m): " value
        value="${value:-$default}"
        if valid_mem_limit "$value"; then
            MEM_LIMIT="$value"
            return 0
        fi
        echo "Invalid format — use a number followed by b/k/m/g (e.g. 1g)." >&2
    done
}
