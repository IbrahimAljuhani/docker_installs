...
Two containers: `dolibarr-app` (Apache + PHP + Dolibarr) and `dolibarr-db` (MariaDB).
...
|---|---|---|---|
| **Dolibarr** | 2 | 1 GB | Small businesses. Easiest to run and to learn; modular, so you enable only invoicing or only stock if that's all you need. |
...
- **`DOLI_CRON` enabled** with a generated key, where upstream defaults it off. Dolibarr's scheduled jobs drive recurring invoices, stock alerts and email reminders; off by default means those silently never run, which in an ERP looks like data loss rather than a missing option.
