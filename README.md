...
This repo installs the **core infrastructure** (`install_dockhub.sh` → Docker CE, Compose, NPM, Portainer, and the shared `main-net` network). Everything else lives under [`services/`](services/README.md) as its own self-contained, independently-deployed folder — run only the ones you actually need. See [`services/README.md`](services/README.md) for the full up-to-date roadmap (38 catalog entries across 12 categories, ✅ = built), quick-start, and conventions — the table there is the source of truth; this section just shows the shape of the repo.
